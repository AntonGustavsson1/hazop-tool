...
| Korsavvikelsereferens | När "Lägg till orsaker på P&ID" öppnas för en avvikelse visas ett nytt avsnitt "Återanvänd från andra avvikelser" i TemplateCausePickerDialog. Orsaker från ANDRA avvikelser i samma nod listas per avvikelse. Varje orsak har: "Referera" (fyller fritextfält med original) och "Invers" (högt↔lågt, stänger↔öppnar, stoppar↔startar, stängd↔öppen). Invers-knappens tooltip visar den inverterade texten. DB: `causes_for_node_excluding_deviation(node_id, deviation_id)`. Inversionslogik: `_INVERSION_MAP` + `invert_cause_text()` i pid_viewer.py. |
