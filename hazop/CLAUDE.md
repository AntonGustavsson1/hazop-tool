...
8. **`tree_panel.py`** — `TreePanel` (the HAZOP hierarchy tree) plus its cause/deviation picker dialogs (`CauseObjectPopup`, `CauseTagPopup`, `RRFPopup`, `FrequencyPickerPopup`, `DeviationPickerPopup`). `StandardCausesPickerPopup` ("Lägg till orsak på P&ID") was removed 2026-08-24 (see NOTES.md) — `TreePanel.add_cause()`/`_add_cause_for_deviation()` now create a blank cause directly (same no-popup pattern as `add_consequence()`/`add_safeguard()`), no dialog.
