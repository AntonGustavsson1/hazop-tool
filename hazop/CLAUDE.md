...
8. **`tree_panel.py`** — `TreePanel` (the HAZOP hierarchy tree) plus its cause/deviation picker dialogs (`CauseTagPopup`, `RRFPopup`, `FrequencyPickerPopup`, `DeviationPickerPopup`). `StandardCausesPickerPopup` ("Lägg till orsak på P&ID") was removed 2026-08-24 (see NOTES.md) — `TreePanel.add_cause()`/`_add_cause_for_deviation()` now create a blank cause directly (same no-popup pattern as `add_consequence()`/`add_safeguard()`), no dialog. `CauseObjectPopup` (the former combined tag+type+object picker) was dead code — retired in favor of the inline editor but never deleted — removed 2026-09-11 dead-code pass.
...
- `effective_f_level(f_level, rrf)` (in `ui_helpers.py`) — reduces F-level by `floor(log10(rrf))` steps. Its old `effective_frequency`/`effective_likelihood` aliases were unused dead code — removed 2026-09-11.
