...
class ReportWordExportTests(unittest.TestCase):
    ...
    def test_tor_includes_compact_active_frequency_catalogue_and_formal_basis(self):
        ...
        self.assertIn('motsvarar ungefär ett tillfälle per elva år', body)
        self.assertIn('BPCS-relaterade initierande orsaker', body)
        self.assertIn('IEC 61511-1:2016, 8.2.2', body)
        self.assertIn('IEC 61511-2:2016, A.8.2.2', body)
        self.assertNotIn('SIL 1 vid hög efterfrågan', body)
        self.assertNotIn('PFH <', body)
        self.assertNotIn('PFDavg', body)
