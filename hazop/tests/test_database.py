...
class DatabaseLayerTests(unittest.TestCase):
    ...
    def test_manual_valve_catalog_uses_two_universal_causes_at_001_per_year(self):
        ...
        valve_id = self.db.conn.execute(
            "SELECT id FROM standard_objects WHERE name='Manuell ventil'").fetchone()['id']
    ...
    def test_onoff_valve_catalog_uses_two_universal_causes_at_009_each(self):
        deviation_id = self.db.conn.execute(
            ...
            "WHERE description='Högt flöde' AND active=1").fetchone()['id']
        valve_id = self.db.conn.execute(
            "SELECT id FROM standard_objects WHERE name='On-off ventil'").fetchone()['id']
        visible = {
    ...
    def test_check_valve_catalog_uses_two_universal_causes_at_001_and_01(self):
        deviation_id = self.db.conn.execute(
            "SELECT id FROM standard_deviations "
            "WHERE description='Lågt flöde' AND active=1").fetchone()['id']
        valve_id = self.db.conn.execute(
            "SELECT id FROM standard_objects WHERE name='Backventil'").fetchone()['id']
        visible = {
            row['description']: row['frequency']
            for row in self.db.standard_causes_for_object(deviation_id, valve_id)
        }
        self.assertEqual({
            'Backventil fastnar stängd': 0.01,
            'Backventil läcker': 0.1,
        }, visible)

    def test_relief_valve_catalog_uses_two_universal_causes_at_001_each(self):
        deviation_id = self.db.conn.execute(
            "SELECT id FROM standard_deviations "
            "WHERE description='Högt tryck' AND active=1").fetchone()['id']
        valve_id = self.db.conn.execute(
            "SELECT id FROM standard_objects WHERE name='Säkerhetsventil / sprängbleck'").fetchone()['id']
        visible = {
            row['description']: row['frequency']
            for row in self.db.standard_causes_for_object(deviation_id, valve_id)
        }
        self.assertEqual({
            'Sprängbleck öppnar för tidigt': 0.01,
    ...
    def test_filter_catalog_uses_two_approved_causes(self):
        deviation_id = self.db.conn.execute(
            "SELECT id FROM standard_deviations "
            "WHERE description='Lågt flöde' AND active=1").fetchone()['id']
        object_id = self.db.conn.execute(
            "SELECT id FROM standard_objects WHERE name='Filter / sil'").fetchone()['id']
        visible = {
            row['description']: row['frequency']
            for row in self.db.standard_causes_for_object(deviation_id, object_id)
        }
        self.assertEqual({
            'Filter / sil igensatt': 0.01,
    ...
    def test_tank_catalog_removes_inflow_and_clears_three_frequencies(self):
        ...
        object_id = self.db.conn.execute(
            "SELECT id FROM standard_objects "
            "WHERE name='Tank / kärl / kolonn'").fetchone()['id']
        visible = {
            row['description']: row['frequency']
            for row in self.db.standard_causes_for_object(deviation_id, object_id)
        }
    ...
    def test_flange_catalog_has_no_active_causes_after_clear(self):
        deviation_id = self.db.conn.execute(
            "SELECT id FROM standard_deviations "
            "WHERE description='Hög temperatur' AND active=1").fetchone()['id']
        object_id = self.db.conn.execute(
    ...
    def test_control_system_catalog_has_no_active_causes_after_clear(self):
        deviation_id = self.db.conn.execute(
            "SELECT id FROM standard_deviations "
            "WHERE description='Hög temperatur' AND active=1").fetchone()['id']
        object_id = self.db.conn.execute(
    ...
    def test_cooling_catalog_has_no_active_causes_after_clear(self):
        deviation_id = self.db.conn.execute(
            "SELECT id FROM standard_deviations "
            "WHERE description='Hög temperatur' AND active=1").fetchone()['id']
        object_id = self.db.conn.execute(
            "SELECT id FROM standard_objects WHERE name='Kylsystem / värmesystem'").fetchone()['id']
        self.assertEqual([], self.db.standard_causes_for_object(deviation_id, object_id))

    def test_mixer_catalog_uses_two_approved_causes(self):
        deviation_id = self.db.conn.execute(
            "SELECT id FROM standard_deviations "
            "WHERE description='Hög temperatur' AND active=1").fetchone()['id']
        object_id = self.db.conn.execute(
            "SELECT id FROM standard_objects WHERE name='Blandare / omrörare'").fetchone()['id']
        visible = {
            row['description']: row['frequency']
            for row in self.db.standard_causes_for_object(deviation_id, object_id)
        }
